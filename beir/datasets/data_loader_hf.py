...
class HFDataLoader:
    ...
    def __init__(self, hf_repo: str = None, hf_repo_qrels: str = None, data_folder: str = None, prefix: str = None, corpus_file: str = "corpus.jsonl", query_file: str = "queries_default.jsonl",
        ...
        if text_type == 'processed_text':
            self.text_type = 'processed_text'
            self.title_type = 'processed_title'
        else:
            self.text_type = 'text'
        ...
        if hf_repo:
            ...
            # (2) fiqa/queries_default.jsonl (format: jsonlines)
            ...
            self.corpus_file = os.path.join(
                data_folder, corpus_file) if data_folder else corpus_file
            self.query_file = os.path.join(
                data_folder, query_file) if data_folder else query_file
            self.qrels_folder = os.path.join(
                data_folder, qrels_folder) if data_folder else None
    ...
    def load(self, split="test") -> Tuple[Dict[str, Dict[str, str]], Dict[str, str], Dict[str, Dict[str, int]]]:
        ...
        if not len(self.corpus):
            ...
            self._load_corpus()
            logger.info("Loaded %d %s Documents.",
                        len(self.corpus), split.upper())
        ...
        filtered_queries = {k: v for k,
                            v in self.queries.items() if k in self.qrels}
    ...
    def _load_corpus(self):
        ...
        corpus_ds = corpus_ds.remove_columns([col for col in corpus_ds.column_names if col not in [
                                             'id', 'text', 'title', 'processed_text', 'processed_title']])

        corpus_dict = {}
        for record in corpus_ds:
            corpus_dict[record['id']] = {
                'text': record[self.text_type],
                'title': record[self.title_type]
            }
    ...
    def _load_queries(self):
        ...
        queries_ds = queries_ds.remove_columns(
            [col for col in queries_ds.column_names if col not in ['id', 'text', 'processed_text']])
    ...
    def _load_qrels(self, split):
        if self.hf_repo:
            qrels_ds = load_dataset(
                self.hf_repo_qrels, keep_in_memory=self.keep_in_memory, streaming=self.streaming)[split]
        else:
            qrels_ds = load_dataset(
